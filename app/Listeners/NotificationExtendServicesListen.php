    ...
    public function handleExtendSalon()
    {
        ...
        foreach ($salons as $item) {
            if (Carbon::now()->between(
                ...
                $message = 'Xin chào bạn, 
                            Chúng tôi thông báo rằng salon của bạn sẽ hết hạn vào ngày ' . Carbon::parse($item->expired_date)->format('d-m-Y') . '
                            .Vì vậy bạn vui lòng ra hạn thêm để có thể tiếp tục duy trì salon của bạn.
                            Cảm ơn bạn đã sử dụng dịch vụ của DRIVCO.';
                ChMessage::create([
                    'from_id' => $bot->id,
                    'to_id' => $item->user_id,
